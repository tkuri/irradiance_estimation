...
class MiRndDataset(BaseDataset):
    """
    ...
    def __getitem__(self, index):
        ...
        for i in range(25):
            ...
            img_path = self.dataroot + '/results/' + path + '/dir_{}_mip2_input.png'.format(tidx)
            srgb_img.append(Image.open(img_path).convert('RGB'))

            Ls_path = self.dataroot + '/results/' + path + '/dir_{}_mip2_L.png'.format(tidx)
            Ls.append(Image.open(Ls_path).convert('RGB'))

            Lt_path = self.dataroot + '/results/' + path + '/dir_{}_mip2_L.png'.format(i)
            Lt.append(Image.open(Lt_path).convert('RGB'))
        ...
        for i in range(25):
            ...
            Ls[i] = L_transform(Ls[i])
            Lt[i] = L_transform(Lt[i])
