...
class CGIntrinsicIIWDataset(BaseDataset):
    """
    ...
    def __init__(self, opt):
        ...
        self.img_paths_cg = make_dataset(self.dataroot + '/intrinsics_final/train_list/', opt.max_dataset_size)
        self.img_paths_iiw = make_dataset(self.dataroot + '/IIW/train_list/', opt.max_dataset_size, iiw=True)
        if len(self.img_paths_cg) == 0 or len(self.img_paths_iiw) == 0:
            raise(RuntimeError("Found 0 images in: " + self.dataroot + '/intrinsics_final/train_list/ or '
                               + self.dataroot + '/IIW/train_list/'+ "\n"
                               "Supported image extensions are: " + ",".join(IMG_EXTENSIONS)))

        assert(self.opt.load_size >= self.opt.crop_size)   # crop_size should be smaller than the size of loaded image
        
        self.A_size = len(self.img_paths_cg)  # get the size of dataset A
        self.B_size = len(self.img_paths_iiw)  # get the size of dataset B
    ...
    def __getitem__(self, index):
        ...
        return {'A': img_cg, 'B': img_iiw, 'A_paths': img_path_cg, 'B_paths': img_path_iiw}
    ...
    def __len__(self):
        ...
        """
        return max(len(self.img_paths_cg), len(self.img_paths_iiw))