...
class Pix2PixTmRegIn2MultiModel(BaseModel):
    ...
    def set_input(self, input):
        ...
        self.real_C_itp = F.interpolate(self.real_C, (self.light_res, self.light_res), mode='bilinear', align_corners=False) # [25, 1, 5, 5]
        self.real_C_itp_flat = self.real_C_itp.view(-1, self.light_res**2, 1) # [25, 25, 1]
        self.real_C_itp = torch.clamp((F.interpolate(self.real_C_itp, (self.real_C.size(-2), self.real_C.size(-1)), mode='nearest')-0.5)/0.5, min=-1.0, max=1.0)  # [25, 256, 256, 1]
    ...
    def forward(self):
        ...
        self.trans_matrix = self.netG(self.real_AC) # [25, 3*25, 256, 256]
        self.ltm_slice00 = torch.clamp((self.trans_matrix[:, [0, 25, 25*2], :, :] - 0.5) / 0.5, min=-1.0, max=1.0) # [25, 3, 256, 256]
        self.ltm_slice12 = torch.clamp((self.trans_matrix[:, [12, 25+12, 25*2+12], :, :] - 0.5) / 0.5, min=-1.0, max=1.0) # [25, 3, 256, 256]
        self.ltm_slice24 = torch.clamp((self.trans_matrix[:, [24, 25+24, 25*2+24], :, :] - 0.5) / 0.5, min=-1.0, max=1.0) # [25, 3, 256, 256]
        trans_matrix = self.trans_matrix.view(-1, self.output_nc*self.light_res**2, (self.trans_matrix.size(-1)*self.trans_matrix.size(-2)))  # [25, 3*25, 256x256]
        trans_matrix = torch.transpose(trans_matrix, 1, 2)  # [25, 256x256, 3*25]
        tmR = trans_matrix[:, :, 0:self.light_res**2] # [25, 256x256, 25]
