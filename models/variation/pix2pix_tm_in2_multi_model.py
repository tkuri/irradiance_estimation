...
class Pix2PixTmIn2Model(BaseModel):
    ...
    def set_input(self, input):
        ...
        self.real_AC = torch.cat([self.real_A, self.real_C], dim=1) # [bn, 4, 256, 256]
        self.image_paths = input['A_paths']
        

    def forward(self):
        """Run forward pass; called by both functions <optimize_parameters> and <test>."""
        trans_matrix = self.netG(self.real_AC) # [bn, 3*16, 256, 256]
        self.ltm_slice00 = torch.clamp((trans_matrix[:, [0, 25, 25*2], :, :] - 0.5) / 0.5, min=-1.0, max=1.0) # [bn, 3, 256, 256]
        self.ltm_slice12 = torch.clamp((trans_matrix[:, [12, 25+12, 25*2+12], :, :] - 0.5) / 0.5, min=-1.0, max=1.0) # [bn, 3, 256, 256]
        self.ltm_slice24 = torch.clamp((trans_matrix[:, [24, 25+24, 25*2+24], :, :] - 0.5) / 0.5, min=-1.0, max=1.0) # [bn, 3, 256, 256]
        trans_matrix = trans_matrix.view(-1, self.output_nc*self.light_res**2, (trans_matrix.size(-1)*trans_matrix.size(-2)))  # [bn, 3*16, 256x256]
        trans_matrix = torch.transpose(trans_matrix, 1, 2)  # [bn, 256x256, 3*16]
        tmR = trans_matrix[:, :, 0:self.light_res**2] # [bn, 256x256, 16]
