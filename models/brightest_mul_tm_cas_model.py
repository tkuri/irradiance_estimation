...
class BrightestMulTmCasModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if opt.LTM:
            ...
            if self.opt.enc_LTM:
                self.dim_LTM = 5
                self.enc_LTM = networks.init_net(networks.IlluminationEncoder(self.light_res**2, 64, dim_LTM), opt.init_type, opt.init_gain, self.gpu_ids)
    ...
    def ltm_module(self, x):
        ...
        ltm = ltm.view(-1, self.dim_LTM, (ltm.size(-1)*ltm.size(-2)))  # [25, 25, 256x256]
    ...
    def eval_label(self):
        ...
        label += self.label_base()['BC'] + self.label_sh()['BC']
        if not self.opt.no_brightness:
            label += self.label_pr(False, '')['BC']

        label += self.label_base()['bcDist'] + self.label_sh()['bcDist']
        if not self.opt.no_brightness:
            label += self.label_pr(False, '')['bcDist']

        label += self.label_base()['baMSE'] + self.label_sh()['baMSE']
        if not self.opt.no_brightness:
            label += self.label_pr(False, '')['baMSE']
