...
class Pix2PixTm2FullRegModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.netG = networks.define_G(G_input, opt.output_nc*self.intermediate_nc, opt.ngf, 'unet_256_lastrelu', opt.norm,
                                      not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)
                                      
        self.netG2 = networks.define_G(G_input, self.intermediate_nc, opt.ngf, 'unet_256_lastrelu', opt.norm,
                                      not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)
    ...
    def set_input(self, input):
        ...
        self.real_C_itp_flat = self.real_C_itp.view(-1, self.light_res**2, 1) # [bn, lsxls, 1]
    ...
    def forward(self):
        ...
        sub_matrix1 = self.netG(src) # [bn, 3xmc, 256, 256]
        sub_matrix2 = self.netG2(src) # [bn, mc, 256, 256]
        sub_matrix2 = F.interpolate(sub_matrix2, (self.light_res, self.light_res), mode='bilinear', align_corners=False)# [bn, mc, ls, ls]

        # Store for LTM Regularization
        self.sub_matrix_1 = sub_matrix1.clone()
        self.sub_matrix_2 = sub_matrix2.clone()

        # Calc each matrix slice for display
        ...
        # Calc LTM(T) with 2 sub matrix
        sub_matrix1 = sub_matrix1.view(-1, sub_matrix1.size(1), sub_matrix1.size(2)*sub_matrix1.size(3)) # [bn, 3xmc, 256x256]
        sub_matrix2 = sub_matrix2.view(-1, sub_matrix2.size(1), sub_matrix2.size(2)*sub_matrix2.size(3)) # [bn, mc, lsxls]
        sub_matrix1 = torch.transpose(sub_matrix1, 1, 2) # [bn, 256x256, 3xmc]
        sm1R = sub_matrix1[:, :, 0:self.intermediate_nc] # [bn, 256x256, mc]
        ...
        self.ltm_slice12 = torch.clamp((ltm[:, 3*12:3*12+3, :, :] - 0.5) / 0.5, min=-1.0, max=1.0) # [bn, 3, 256, 256]
