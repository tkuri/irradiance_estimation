...
class Pix2PixTm2In3Model(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.visual_names = ['real_A', 'fake_B', 'real_B', 'real_C', 'real_C_itp2', 'real_D', 'matrix_1', 'matrix_2']
        ...
        #                               not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)
        self.netG = networks.define_G(opt.input_nc + opt.input2_nc + opt.input3_nc, opt.output_nc, opt.ngf, 'unet_256_lastrelu', opt.norm,
                                      not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)
                                      
        self.netG2 = networks.define_G(opt.input_nc + opt.input2_nc + opt.input3_nc, 1, opt.ngf, 'unet_256_lastrelu', opt.norm,
                                      not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)
    ...
    def forward(self):
        ...
        tmR = sub_matrix1[:, 0:256**2, :] # [1, 256x256, lsxls]
        tmG = sub_matrix1[:, 256**2:(256**2)*2, :]
        tmB = sub_matrix1[:, (256**2)*2:(256**2)*3, :]
        bufR = torch.matmul(tmR, sub_matrix3) # [1, 256x256, 1]
